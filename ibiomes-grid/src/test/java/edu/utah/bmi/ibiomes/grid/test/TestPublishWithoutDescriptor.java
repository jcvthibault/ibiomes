...
/**
 * Test suite to publish experiment to iBIOMES without specifying any XML parser rule descriptor
 * @author Julien Thibault, University of Utah
 *
 */
public class TestPublishWithoutDescriptor
{
	...
	private static final String[] charmmCollections = {
		TestCommon.TEST_DATA_DIR + "/charmm/embnet-tutorial"};
	
	private static final String[] namdCollections = {
	...
	public void publish() throws Exception
	{
		...
		try{
			...
			for (int c=0;c<gaussianCollections.length;c++){
				...
				ExperimentFolder experimentFolder = expFactory.parseDirectoryForExperimentWorkflowAndMetadata( Software.GAUSSIAN, null, null );
			...
			for (int c=0;c<nwchemCollections.length;c++){
				...
				ExperimentFactory expFactory = new ExperimentFactory(dir.getAbsolutePath());
				ExperimentFolder experimentFolder = expFactory.parseDirectoryForExperimentWorkflowAndMetadata( Software.NWCHEM, null, null );
			...
			for (int c=0;c<amberCollections.length;c++){
				...
				ExperimentFactory expFactory = new ExperimentFactory(dir.getAbsolutePath());
				ExperimentFolder experimentFolder = expFactory.parseDirectoryForExperimentWorkflowAndMetadata( Software.AMBER, null, null );
				publisher.publishExperiment(experimentFolder.getFileDirectory(), IBIOMES_DIR + "/md_amber_"+ dir.getName(), true);
			}

			// CHARMM MD
			for (int c=0;c<charmmCollections.length;c++){
				System.out.println("Publishing CHARMM experiment "+charmmCollections[c]+"...");
				File dir = new File(charmmCollections[c]);
				ExperimentFactory expFactory = new ExperimentFactory(dir.getAbsolutePath());
				ExperimentFolder experimentFolder = expFactory.parseDirectoryForExperimentWorkflowAndMetadata( Software.CHARMM, null, null );
				publisher.publishExperiment(experimentFolder.getFileDirectory(), IBIOMES_DIR + "/md_charmm_"+ dir.getName(), true);
			}


			// NAMD MD
			for (int c=0;c<namdCollections.length;c++){
				System.out.println("Publishing NAMD experiment "+namdCollections[c]+"...");
				File dir = new File(namdCollections[c]);
				ExperimentFactory expFactory = new ExperimentFactory(dir.getAbsolutePath());
				ExperimentFolder experimentFolder = expFactory.parseDirectoryForExperimentWorkflowAndMetadata( Software.NAMD, null, null );
				publisher.publishExperiment(experimentFolder.getFileDirectory(), IBIOMES_DIR + "/md_namd_"+ dir.getName(), true);
			}
		/*	
			System.out.println("\n======================================================================");
